...
```mermaid
flowchart TD
    A[Policy JSON] --> B[Clean and create documents]
    B --> C[Chunks: 500 characters, 50 overlap]
    C --> D[MiniLM local embeddings]
    D --> E[FAISS vector index]

    U[User question] --> F[Streamlit application]
    T[Evaluation questions] --> G[Evaluation runner]
    F --> H[Top-3 similarity retrieval]
    G --> H
    E --> H
    H --> I[Grounded prompt plus context]
    I --> J[Gemini answer generation]
    J --> K[Answer, sources and latency]
    K --> F
    K --> G
    G --> L[Gemini judge plus RAGAS]
    L --> M[evaluation_results.json]
    J -. optional traces .-> N[LangSmith]
```
